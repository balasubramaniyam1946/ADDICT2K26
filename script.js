...
// Rules and Content Data
const rulesData = {
    'ADZAP': ["1.Each team shall consist of a maximum of two (2) members.",
              "2.Two to three topics will be provided on the spot to each team.",
              "3.Participants are required to design and advertise a product based on the given topic.",
              "4.A total of 30 minutes will be allotted for poster creation.",
              "5.10 minutes will be provided for presentation and advertisement of the created product."
            ],
    'ADZAP-RULES': ["1.The use of AI-generated images or graphics is strictly prohibited.",
                    "2.Participants must create their poster only on the platform provided by the organizers.",
                    "3.The presentation must be delivered exclusively in English.",
                    "4.Posters and presentations must not include political, religious, or controversial content.",
                    "5.The poster creation process must begin from scratch; the use of pre-designed templates, previously created content, or stored materials is not permitted."],
...
function openModal(title, venue, coordinator) {
    document.getElementById('modalTitle').innerText = title;
    document.getElementById('modalVenue').innerText = `VENUE: ${venue}`;
    document.getElementById('modalCoords').innerText = `COORD: ${coordinator}`;
    document.getElementById('callCoordinator').href = `tel:${coordinator.phone}`;
    document.getElementById('whatsappCoordinator').href =`https://wa.me/${coordinator.phone}`;
    
    const list = document.getElementById('modalRules');
    list.innerHTML = "";
    (rulesData[title] || []).forEach(r => {
